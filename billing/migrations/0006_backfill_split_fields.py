...
def backfill(apps, schema_editor):
    ...
    default_pct = (
        bs.default_admin_percent
        if bs and bs.default_admin_percent is not None
        else Decimal("20.00")
    )
    ...
    TabCharge.objects.filter(application_fee__isnull=True).update(
